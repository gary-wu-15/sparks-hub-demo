...
function sparksHero() {
  const wallets = heroWalletVisibility(data.recommendations, data.rankingSchema, data.previewOverrides);
...
function render() {
  ...
  app.innerHTML = `<header class="site-header">
    ...
    ${sparksHero()}
    ${data.previewOverrides.walletsInHero ? '' : walletPlacements(data.recommendations, data.rankingSchema).filter((item) => item.rank > 2 || item.sourceComponent === 'credit_card_offers').map((item) => standaloneWallet(item.component)).join('')}
